"""
MODULE 1: NORTH AMERICA HARMONIZATION (MASS-PRESERVING EDITION)
Includes pre-flight file validation, empty intersection crash protection, 
and a 10km Focal Window algorithm to perfectly conserve SPAM census mass.
"""
import sys
import numpy as np
import rasterio
...
import gc
import warnings
from scipy.ndimage import uniform_filter  # NEW: The focal window engine

warnings.filterwarnings("ignore")

# ── 1. Setup & Global Grid Definition ─────────────────────────────────────────
path_gee   = Path("path_input/test_output_250m/")
path_calc  = Path("mod1_final_output_250m/") 
...
# North America bounding box limits
NA_EXTENT = {"xmin": -125, "xmax": -65, "ymin": 25, "ymax": 50}
min_col = math.floor((NA_EXTENT["xmin"] - final_ext["xmin"]) / resolution / BLOCK_SIZE)
max_col = math.ceil((NA_EXTENT["xmax"] - final_ext["xmin"]) / resolution / BLOCK_SIZE)
min_row = math.floor((final_ext["ymax"] - NA_EXTENT["ymax"]) / resolution / BLOCK_SIZE)
max_row = math.ceil((final_ext["ymax"] - NA_EXTENT["ymin"]) / resolution / BLOCK_SIZE)

VALID_X_TILES = range(max(0, min_col), min(x_blocks, max_col + 1))
VALID_Y_TILES = range(max(0, min_row), min(y_blocks, max_row + 1))
...
print("=" * 60)
print("MODULE 1: MASS-PRESERVING DASYMETRIC HARMONIZATION (NA TEST)")
print("=" * 60)

# ── 2. PRE-FLIGHT VALIDATION CHECK ────────────────────────────────────────────
def run_pre_flight_check():
    print("Executing Pre-Flight Data Scan...")
    required_prefixes = [
        "esa_cropland_fraction_250m",
...
total_active_tiles = len(VALID_X_TILES) * len(VALID_Y_TILES)
active_count = 0

print(f"Targeting {total_active_tiles} specific NA tiles.")

for y in range(y_blocks):
    for x in range(x_blocks):
        if x not in VALID_X_TILES or y not in VALID_Y_TILES:
            continue
            
        active_count += 1
        print(f"  Processing Active NA Tile {active_count}/{total_active_tiles}...      ", end='\r')
        ...
        # ── 0. Load ESA Cookie-Cutter & Calculate Focal Weights ──
        ...
        # Calculates total dirt in the local 10km neighborhood for every pixel.
        WINDOW_SIZE = 40 
        local_total_capacity = uniform_filter(pixel_capacity_ha, size=WINDOW_SIZE, mode='reflect') * (WINDOW_SIZE ** 2)
        
        # Prevent division by zero where there is no cropland at all
        safe_local_capacity = np.where(local_total_capacity > 0, local_total_capacity, 1.0)
        
        # Calculate this specific 250m pixel's proportional weight of the local 10km dirt
        ...
        # Clean up water/city areas explicitly to guarantee zero overlap where ESA is 0.0
...
print("NA Test Harmonization Complete. Outputs saved.")