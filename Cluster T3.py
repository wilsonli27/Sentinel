...
def plot_enhanced_visualization(processor, cluster_result, start_date, end_date, analyses):
    ...
    - Time series with uncertainty envelopes
    ...
    valid_clusters = {}
    for cluster_id, data in cluster_data.items():
        # Convert mean to numpy array, replacing None with nan
        mean_list = data['mean']
        try:
            ...
            # Check if we have at least some valid data (not all NaN)
    ...
    print(f"    Visualizing {len(valid_clusters)} clusters with valid data...")
    
    # Determine grid size based on valid clusters - use 3 columns for better spacing
    n_cols = 3
    n_rows = int(np.ceil(len(valid_clusters) / n_cols)) + 1  # Extra row for summary plots
    
    # Increase figure size significantly for better readability
    ...
    # Plot 1: Cluster distribution (first plot in first row)
    ...
    bars = ax1.bar(bins, counts, color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ax1.set_xlabel('Cluster ID', fontweight='bold', fontsize=16)
    ax1.set_ylabel('Pixel Count', fontweight='bold', fontsize=16)
    ax1.set_title('Cluster Distribution', fontweight='bold', fontsize=18, pad=10)
    ax1.grid(True, alpha=0.3, axis='y')
    ax1.tick_params(labelsize=14)
    
    # Plot 2: Seasonality summary (spans remaining columns in first row)
    ax2 = fig.add_subplot(gs[0, 1:])
    ax2.axis('off')
    
    summary_text = "SEASONALITY ANALYSIS\n" + "="*55 + "\n\n"
    ...
    summary_text += f"Single-season clusters:  {single_season}\n"
    summary_text += f"Biannual clusters:       {biannual}\n"
    summary_text += f"Complex patterns:        {complex_pattern}\n"
    summary_text += f"No clear pattern:        {no_pattern}\n\n"
    summary_text += "="*55 + "\n\n"
    
    # Show top 5 clusters with patterns
    clusters_with_peaks = [(cid, a) for cid, a in analyses.items() if a['n_peaks'] > 0]
    clusters_with_peaks.sort(key=lambda x: x[1]['n_pixels'], reverse=True)
    
    for cluster_id, a in clusters_with_peaks[:5]:
        summary_text += f"Cluster {cluster_id}: {a['pattern']}\n"
        if len(a['peak_doys']) > 0:
            peak_str = ', '.join([f"{int(d)}" for d in a['peak_doys']])
            summary_text += f"  Peak DOYs: {peak_str}\n"
        if len(a['crop_free_periods']) > 0:
            summary_text += f"  Crop-free periods: {len(a['crop_free_periods'])}\n"
        summary_text += "\n"
    ...
    # Plots 3+: Individual cluster time series (starting from row 1)
    for idx, cluster_id in enumerate(sorted(valid_clusters.keys())):
        row = 1 + (idx // n_cols)
        col = idx % n_cols
        ...
        if not np.any(valid_mask):
            # Skip this cluster if all data is NaN
        ...
        # Replace any remaining NaN in std with 0
        std_valid = np.nan_to_num(std_valid, nan=0.0)
        
        # Plot mean line with thicker line for better visibility
        ax.plot(doy_valid, mean_valid, 'k-', linewidth=2.5, label='Mean NDVI', zorder=10)
        
        # Plot ±1σ envelope
        lower_1sig = np.maximum(mean_valid - std_valid, 0)  # Clamp to 0
        upper_1sig = np.minimum(mean_valid + std_valid, 1)  # Clamp to 1
        ax.fill_between(doy_valid, lower_1sig, upper_1sig,
                        color='blue', alpha=0.35, label='±1σ', zorder=5)
        
        # Plot ±2σ envelope
        lower_2sig = np.maximum(mean_valid - 2*std_valid, 0)  # Clamp to 0
        upper_2sig = np.minimum(mean_valid + 2*std_valid, 1)  # Clamp to 1
        ax.fill_between(doy_valid, lower_2sig, upper_2sig,
                        color='blue', alpha=0.18, label='±2σ', zorder=3)
        
        # Mark peaks with larger, more visible markers
        if len(analysis['peak_doys']) > 0:
            peak_doys_array = np.array(analysis['peak_doys'])
            peak_ndvi_array = np.array(analysis['peak_ndvi'])
            # Only plot peaks that are within our valid data range
            ...
            if np.any(valid_peaks_mask):
                ax.plot(peak_doys_array[valid_peaks_mask], peak_ndvi_array[valid_peaks_mask], 
                       'r*', markersize=20, markeredgewidth=1.5, markeredgecolor='darkred',
                       label='Peaks', zorder=15)
        
        # Mark crop-free periods with more visible shading
        ...
        # Add legend only to first plot with better formatting
        if idx == 0:
