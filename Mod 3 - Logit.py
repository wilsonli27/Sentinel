"""
MODULE 3: GLOBAL CA PIPELINE (ALL-IN-ONE)
Dynamically calculates the Logit using the fresh Mod 2 data, 
solves country thresholds, and allocates all outputs perfectly.
Matches Luxembourg Regional Test logic perfectly.
"""
import numpy as np
import rasterio
import pandas as pd
...
print("=" * 60)
print("MODULE 3: GLOBAL CA ALLOCATION (FINAL PIPELINE)")
print("=" * 60)

try:
    # 🚀 Loading the FRESH files made by the new Mod 2
...
print("\n[STEP 1]: Calculating Dynamic Global Logit & Sanitizing...")
# Open Logit file in write mode to save it as we calculate it
...
with rasterio.Env(GDAL_CACHEMAX=2000000000, GDAL_NUM_THREADS='ALL_CPUS'):
    for idx, (ji, window) in enumerate(blocks):
        ...
        # 🚀 2. Strict Sanitizer (No 1600x division)
        ...
        if np.any(valid_mask):
            ...
            # 🚀 3. Pure Physical Area Sum (No Logit Multiplier)
            pot_ca = np.nansum(grains[:, valid_mask], axis=0) 
            valid_pot_ca.append(pot_ca)

out_logit.close() # Save the new Logit map securely
...
print("\n\n[STEP 2]: Reading GPKG Bridge & Solving Quotas...")
...
# 🚀 The Master Dictionary translating FAOSTAT directly to ISO3
fao_iso3_map = {
...
# Automated fallback using UN GitHub list for remaining countries
...
try:
    ...
    print(f"Note: Could not fetch backup UN codes. Standard dictionary applied. ({e})")

# Map ISO3 to your Raster IDs using the GPKG bridge
...
# We re-open the newly created logit file in read mode
...
with rasterio.Env(GDAL_CACHEMAX=2000000000, GDAL_NUM_THREADS='ALL_CPUS'):
    for idx, (ji, window) in enumerate(blocks):
        ...
        # Sanitizer again for safe writing
        invalid_mask = (grains > 6.25) | (grains < 0)
        grains[invalid_mask] = np.nan
